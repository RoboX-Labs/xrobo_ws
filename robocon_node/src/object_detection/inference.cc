#include "robocon_node/object_detection/inference.h"
...
namespace yolo
{
    ...
    std::vector<Detection> Inference::RunInference(cv::Mat &frame)
    {
        ...
        PostProcessing(frame, detections); // Postprocess the inference results

        return detections; // Return the detection results
    }
    ...
    void Inference::PostProcessing(cv::Mat &frame, std::vector<Detection> &detections)
    {
        ...
        const float *detections_data = inference_request_.get_output_tensor().data<const float>();
        const cv::Mat detection_outputs(model_output_shape_, CV_32F, (float *)detections_data); // Create OpenCV matrix from output tensor

        // Iterate over detections_data and collect class IDs, confidence scores, and bounding boxes
        ...
        // Collect final detections_data after NMS
        for (uint8_t i = 0; i < NMS_result.size(); ++i)
        {
            Detection result;
            ...
            detections.push_back(result); // Add the detection to the results vector
